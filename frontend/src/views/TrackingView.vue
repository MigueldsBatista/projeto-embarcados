...
import { useLiveTracking } from '@/composables/useLiveTracking';
import { useDiscovery } from '@/composables/useDiscovery';
import { useHeatmap } from '@/composables/useHeatmap';
import AppNavbar from '@/components/AppNavbar.vue';
import ActivityChart from '@/components/ActivityChart.vue';
import { MapPin, Signal, Clock, User, Radio, Copy, Bug, BarChart3, X, LayoutGrid, AreaChart } from 'lucide-vue-next';

const { data: liveTracking, isLoading: isLoadingLive } = useLiveTracking();
const { data: discovery, isLoading: isLoadingDiscovery } = useDiscovery();
const showDebug = ref(false);
const selectedStudentMac = ref<string | null>(null);
const chartType = ref<'bar' | 'boxPlot' | 'heatmap'>('bar');

const { data: heatmapData, isLoading: isLoadingHeatmap } = useHeatmap(selectedStudentMac);
  ...
  <div class="min-h-screen pb-12 bg-slate-50/50">
    ...
    <main class="max-w-7xl mx-auto px-4 space-y-8">
      <header class="flex justify-between items-start">
        <div>
          <h1 class="text-2xl font-bold text-slate-800">Real-Time Tracking</h1>
          <p class="text-slate-500 text-sm">Approximate child location via WiFi proximity.</p>
        </div>
      ...
      <!-- Activity Section -->
      <section v-if="selectedStudentMac" class="animate-in fade-in slide-in-from-top-4 duration-300">
        <div class="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden">
          <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="font-bold text-slate-800">Scanner Distribution: {{ liveTracking?.find(s => s.mac === selectedStudentMac)?.student_name }}</h2>
                <p class="text-xs text-slate-500">Activity distribution by zone and scanner in the last 24h</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <div class="flex bg-slate-200 p-1 rounded-xl">
                ...
                <button @click="chartType = 'boxPlot'" 
              ...
              <button @click="selectedStudentMac = null" class="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-all">
                <X class="h-5 w-5" />
              </button>
            </div>
          </div>
          <div class="p-6 space-y-8">
            <div v-if="isLoadingHeatmap" class="h-64 flex items-center justify-center">
              <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div>
            </div>
            <template v-else-if="heatmapData && heatmapData.length > 0">
              <div v-for="scanner in heatmapData" :key="scanner.scanner_id" class="space-y-2">
                <div class="flex items-center gap-2 px-1">
                  <Signal class="h-4 w-4 text-slate-400" />
                  <h3 class="text-sm font-bold text-slate-600 uppercase tracking-tight">{{ scanner.scanner_name }}</h3>
                </div>
                <ActivityChart :series="chartType === 'boxPlot' ? scanner.boxplot_series : scanner.series" :type="chartType" />
              </div>
            </template>
            <p v-else class="text-center py-12 text-slate-400 italic">No movement data for this student in the last 24h.</p>
          </div>
        </div>
      </section>
      ...
      <section class="space-y-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-lg font-bold text-slate-800">Monitored Students</h2>
        </div>
        ...
        <div v-else-if="!liveTracking || liveTracking.length === 0"
          ...
          <p class="text-slate-500 font-medium">No students with linked trackers.</p>
          <p class="text-xs text-slate-400 mt-1">Link a MAC address in the student registry to start monitoring.</p>
        </div>
          ...
          <div v-for="item in liveTracking" :key="item.mac"
            ...
            <div class="grid grid-cols-2 gap-3">
              <div :class="['p-3 rounded-xl border text-center transition-colors', getZoneColor(item.zone)]">
                <p class="text-[9px] uppercase font-black opacity-60 mb-0.5">Zone</p>
                <p class="font-bold text-sm">{{ item.zone }}</p>
              </div>
              <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
                <p class="text-[9px] uppercase font-black text-slate-400 mb-0.5">Signal</p>
...
...
      ...
      <section class="space-y-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-lg font-bold text-slate-800">Nearby Devices (New)</h2>
        </div>
        ...
        <div v-else-if="sortedDiscovery.length === 0" class="p-8 bg-white/50 border-2 border-dashed border-slate-200 rounded-2xl text-center">
          ...
          <p class="text-sm text-slate-500 italic">Searching for nearby tags...</p>
          <p class="text-[10px] text-slate-400 mt-1">Turn on a Wemos to see it appear here.</p>
        </div>
      ...
      <div class="bg-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl shadow-indigo-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-10">
          <div>
            ...
            <p class="text-sm opacity-90 leading-relaxed text-indigo-50">Strong signal (>-55dBm). The child is very close to the scanner (same room).</p>
          </div>
          <div>
            <p class="font-black text-blue-300 uppercase text-xs mb-1">Near</p>
            <p class="text-sm opacity-90 leading-relaxed text-indigo-50">Medium signal (>-75dBm). The child is nearby or in an adjacent room.</p>
          </div>
          <div>
            <p class="font-black text-yellow-300 uppercase text-xs mb-1">Far</p>
            <p class="text-sm opacity-90 leading-relaxed text-indigo-50">Weak signal. The child is moving away or there are many obstacles in the path.</p>
          </div>
        </div>
      </div>
    </main>

...
import { ref } from 'vue';
import { useScanners } from '@/composables/useScanners';
import { useUpdateScanner } from '@/composables/useUpdateScanner';
import AppNavbar from '@/components/AppNavbar.vue';
import { Radio, Edit2, Check, X, Clock, Loader2 } from 'lucide-vue-next';
...
<template>
  <div class="min-h-screen pb-12 bg-slate-50/50">
    ...
    <main class="max-w-7xl mx-auto px-4 space-y-8">
      <div>
        <h1 class="text-2xl font-bold text-slate-800">Manage Scanners</h1>
        <p class="text-slate-500 text-sm">Configure monitoring points (ESP32) spread throughout the school.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="scanner in scanners" :key="scanner.id" 
          ...
          <div v-if="editingId === scanner.identifier" class="space-y-3">
            ...
            <div class="flex gap-2">
              <button @click="saveEdit(scanner.identifier)" 
          ...
          <div v-else>
            <h3 class="text-lg font-bold text-slate-800 mb-1">{{ scanner.name }}</h3>
            <button @click="startEdit(scanner)" class="text-xs text-indigo-600 font-bold flex items-center gap-1 hover:underline">
      ...
      <div v-if="!isLoading && scanners?.length === 0" class="text-center py-24 bg-white rounded-3xl border-2 border-dashed border-slate-200">
        <Radio class="h-12 w-12 text-slate-300 mx-auto mb-4" />
        <p class="text-slate-500 font-bold">No scanners detected.</p>
        <p class="text-sm text-slate-400">Scanners will appear here automatically as soon as they detect a tag for the first time.</p>
      </div>
    </main>
  </div>
</template>

...
<template>
  <div class="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-100">
    <table class="w-full text-left border-collapse">
      ...
      <tbody>
        <tr v-for="card in cards" :key="card.id" class="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
          <td class="p-4">
          ...
          <td class="p-4">
            <span :class="['inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize', getRoleClass(card.role)]">
              <component :is="card.role === 'admin' ? Shield : User" class="w-3 h-3 mr-1" />
              {{ card.role }}
            </span>
          </td>
          ...
          <td class="p-4 text-right">
            <div class="flex justify-end space-x-2">
              ...
              <button @click="emit('delete', card.uid)" 
